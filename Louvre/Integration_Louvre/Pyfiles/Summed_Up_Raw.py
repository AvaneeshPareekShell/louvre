...
VBRP = readTcDbTable(spark,"RAW_TERMINAL_COCKPIT.VBRP").filter((col("vtweg").isin(li_vtweg)) & (col("vkorg").isin(li_vkorg))).select('VBELN','POSNR','AUBEL','OIC_PTRIP','oid_miscdl','vkorg').dropDuplicates().withColumnRenamed("POSNR","VBRP_Position_Number").withColumnRenamed("VBELN","Invoice_Number").withColumnRenamed("oid_miscdl","VBRP_Sales_Contract_Number")
...
KNA1_Filtered = KNA1.filter(length(KNA1.NAME1) != 0).withColumn("kunnr",sf.regexp_replace('kunnr', r'^[0]*', ''))
...
#df_merged_TC_NextGen.filter(df_merged_TC_NextGen.vbeln == '0322858554').display()
...
df_mergerd_VBRP = df_merged_TC_NextGen_add_Fields.join(VBRP,df_merged_TC_NextGen_add_Fields["MSEG_SO"] ==  VBRP["AUBEL"],"left").drop(VBRP.AUBEL)
...
df_merged_TC_NextGen_Austria_Contracts = df_mergerd_BSARK_Update.withColumn("Contract_Start_Period", when(df_merged_TC_NextGen_add_Fields.vbeln.isin('0322446905','0322581121','0322581137','0322581138','0322581181','0322581184','0322581186','0322581205','0322581206','0322581207','0322581210','0322581211','0322581213','0322581214','0322581338','0322581457','0322583862','0322915343') & df_mergerd_BSARK_Update.BSARK_E.isin('TTFD','TTTT','TTTS','TTTM','TTTI'),df_mergerd_BSARK_Update.Lifting_Period).otherwise(df_mergerd_BSARK_Update.Contract_Start_Period))

# COMMAND ----------

df_merged_TC_NextGen_select = df_merged_TC_NextGen_Austria_Contracts.select('ZMENG','ktext','ERDAT','auart','OID_SHIP','kunnr','GUEEN','GUEBG','bukrs','MATNR','VBAP_WERKS','lifting_in_KG','lifting_in_L15','VSBED','OIHANTYP','POSNR','aedat','MBLNR','OIVBELN','abgru','ZIEME','vbeln','lifting_in_to','lifting_in_cbm','VBAP_bukrs','MSEG_SO','knumv','BSARK_E','Contract_Start_Period','Contract_End_Period','Pricing_Scheme').dropDuplicates()

# COMMAND ----------

# trim the leading o's for material number to match with mrd_material tables
#df_merged_TC_NextGen_select_rn = df_merged_TC_NextGen_select.withColumn("tr_matnr",sf.regexp_replace('MATNR', r'^[0]*', ''))
...
TC_NextGen_Merged = df_merged_TC_NextGen_select.join(t001w_cc,df_merged_TC_NextGen_select["vbap_werks"] ==  t001w_cc["Plant"],"left").drop(t001w_cc.Plant)
...
Summed_Up_Raw = TC_NextGen_Merged.exceptAll(TC_NextGen_Merged_RejDeals)

# COMMAND ----------

#Summed_Up_Raw = TC_NextGen_Merged_Final.select('ZMENG','ktext','ERDAT','Pricing_Scheme','AUART','OID_SHIP','kunnr','sold_to_name','BSARK_E','GUEEN','GUEBG','bukrs','MATNR','VBAP_WERKS','lifting_in_KG','lifting_in_L15','VSBED','OIHANTYP','POSNR','aedat','MBLNR','OIVBELN','abgru','ZIEME','Ship_To_Name','vbeln','lifting_in_to','lifting_in_cbm','VBAP_bukrs','Contract_Start_Period','Contract_End_Period').dropDuplicates()
